...
db::manager_info_result_t update_info(db::database &db, db::value const &cur_save_id, db::value const &last_save_id);
...
// DB上の関連のデータをパージする
db::update_result_t purge_relations(db::database &db, std::string const &table_name, std::string const &src_table_name);
// DB上のデータをパージする
