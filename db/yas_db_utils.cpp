...
db::update_result_t db::create_table(db::database_ptr const &db, std::string const &table_name,
                                     std::vector<std::string> const &fields) {
    return db->execute_update(db::create_table_sql(table_name, fields));
}

db::update_result_t db::alter_table(db::database_ptr const &db, std::string const &table_name,
                                    std::string const &field) {
    return db->execute_update(db::alter_table_sql(table_name, field));
}

db::update_result_t db::drop_table(db::database_ptr const &db, std::string const &table_name) {
    return db->execute_update(db::drop_table_sql(table_name));
}

db::update_result_t db::create_index(db::database_ptr const &db, std::string const &index_name,
...
db::update_result_t db::drop_index(db::database_ptr const &db, std::string const &index_name) {
    return db->execute_update(db::drop_index_sql(index_name));
}

db::update_result_t db::begin_transaction(db::database_ptr const &db) {
    return db->execute_update("BEGIN EXCLUSIVE TRANSACTION");
}

db::update_result_t db::begin_deferred_transaction(db::database_ptr const &db) {
    return db->execute_update("BEGIN DEFERRED TRANSACTION");
}

db::update_result_t db::commit(db::database_ptr const &db) {
    return db->execute_update("COMMIT TRANSACTION");
}

db::update_result_t db::rollback(db::database_ptr const &db) {
...
db::update_result_t db::start_save_point(db::database_ptr const &db, std::string const &name) {
    if (name.size() == 0) {
...
db::update_result_t db::release_save_point(db::database_ptr const &db, std::string const &name) {
    if (name.size() == 0) {
        return db::update_result_t{db::error{db::error_type::invalid_argument}};
    }
...
db::update_result_t db::rollback_save_point(db::database_ptr const &db, std::string const &name) {
    if (name.size() == 0) {
        return db::update_result_t{db::error{db::error_type::invalid_argument}};
    }
...
db::update_result_t db::in_save_point(db::database_ptr const &db, std::function<void(bool &rollback)> const function) {
...
bool db::column_exists(db::database_ptr const &db, std::string column_name, std::string table_name) {
...
db::select_result_t db::select(db::database_ptr const &db, db::select_option const &option) {
...
db::select_single_result_t db::select_single(db::database_ptr const &db, db::select_option option) {
