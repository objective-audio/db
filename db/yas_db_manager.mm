...
namespace db {
    ...
    object_data_result_t fetch_object_data(db::database &db, db::relation_map_t const &rel_models,
...
struct db::manager::impl : public base::impl, public object_observable::impl {
                    ...
                    if (auto select_result = db::select(
                        ...
                        if (update_info_result) {
                            ...
                            if (info.count(db::version_field) == 0) {
                                state = db::manager::result_t{
                                    db::manager::error{db::manager::error_type::version_not_found}};
                            } else {
    ...
    void execute_insert(
        ...
        execute([preparation = std::move(preparation), completion = std::move(completion),
    ...
    void execute_fetch_object_datas(
        ...
        std::function<void(db::manager::result_t &&state, db::object_data_vector_map_t &&fetched_datas)> &&completion,
        operation_option_t &&option) {
        execute([completion = std::move(completion), preparation = std::move(preparation),
